...

{
    ...
    my $regular_profiles_setup = $provider->{profiles}->[0]->{profile};
    my $callee_profiles_setup = $provider->{profiles}->[1]->{profile};
    my $caller = Utils::Api::setup_subscriber($provider,$caller_profiles_setup,undef,{ cc => 888, ac => '1<n>', sn => '<t>' });
    my $callee = Utils::Api::setup_subscriber($provider,$callee_profiles_setup,undef,{ cc => 888, ac => '2<n>', sn => '<t>' });
