...
foreach my $prepaid ((0,1)) { #0,1)) {
    foreach my $balance ((undef,5,65)) { #undef,5,65)) {
        ...
        my $caller_ratio = _get_free_ratio(Utils::Api::datetime_from_string($caller->{customer}->{create_timestamp},$tz),
            Utils::Api::datetime_from_string($caller->{first_interval}->{start},$tz),
            Utils::Api::datetime_from_string($caller->{first_interval}->{stop},$tz));
        ...
        my $callee_ratio = _get_free_ratio(Utils::Api::datetime_from_string($callee->{customer}->{create_timestamp},$tz),
            Utils::Api::datetime_from_string($callee->{first_interval}->{start},$tz),
            Utils::Api::datetime_from_string($callee->{first_interval}->{stop},$tz));
