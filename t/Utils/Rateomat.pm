...
sub create_prepaid_costs_cdrs {
	...
	eval {
		...
		if ($is_ary) {
			_begin_transaction($dbh);
			$result = _create_prepaid_cost_cdrs($dbh,$cdrs);
			_commit_transaction($dbh);
		} else {
			$result = _create_prepaid_cost_cdr($dbh,@_);
		}
	};
	if ($@) {
		diag($@);
...
sub _get_prepaid_costs_cdrs {
	...
	if ($dbh) {
		...
		foreach my $cdr (@$cdrs) {
			...
			$sth->finish();
			$prepaid_costs_cdr = { cdr => $cdr, prepaid_costs => $prepaid_costs };
			$prepaid_costs_cdr_map{$prepaid_costs->{id}} = $prepaid_costs_cdr if $created;
