...
sub _delete_cdrs {
	...
	if ($dbh) {
		...
		_delete_cdr_cash_balance_data($dbh,$ids);
		_delete_cdr_time_balance_data($dbh,$ids);
		_delete_cdr_relation_data($dbh,$ids);
...
sub _delete_cdr_relation_data { # as long as no triggers are present
	my ($dbh,$ids) = @_;
	if ($dbh) {
		my $sth = $dbh->prepare('DELETE FROM accounting.cdr_relation_data WHERE cdr_id IN ('.substr(',?' x scalar @$ids,1).')')
			or die("Error preparing delete cdr relation data statement: ".$dbh->errstr);
		$sth->execute(@$ids);
		if ($sth->rows > 0) {
            diag($sth->rows . ' cdr cdr_relation_data rows deleted');
        }
		$sth->finish();
	}
}

sub _delete_cdr_cash_balance_data { # as long as no triggers are present
	my ($dbh,$ids) = @_;
	if ($dbh) {
		my $sth = $dbh->prepare('DELETE FROM accounting.cdr_cash_balance_data WHERE cdr_id IN ('.substr(',?' x scalar @$ids,1).')')
			or die("Error preparing delete cdr cash balance data statement: ".$dbh->errstr);
		$sth->execute(@$ids);
		if ($sth->rows > 0) {
            diag($sth->rows . ' cdr cdr_cash_balance_data rows deleted');
        }
		$sth->finish();
	}
}

sub _delete_cdr_time_balance_data { # as long as no triggers are present
	my ($dbh,$ids) = @_;
	if ($dbh) {
		my $sth = $dbh->prepare('DELETE FROM accounting.cdr_time_balance_data WHERE cdr_id IN ('.substr(',?' x scalar @$ids,1).')')
			or die("Error preparing delete cdr time balance data statement: ".$dbh->errstr);
		$sth->execute(@$ids);
		if ($sth->rows > 0) {
            diag($sth->rows . ' cdr cdr_time_balance_data rows deleted');
        }
		$sth->finish();
	}
}
