...
my $failed_cdr_max_retries = 2;
my $failed_cdr_retry_delay = 30;
...
EOS
	...
	$sth_prepaid_costs_count = $acctdbh->prepare(
		"SELECT count(cnt.id) FROM (SELECT id FROM accounting.prepaid_costs LIMIT " . $prepaid_costs_cache_limit . ") AS cnt"
...
sub get_prepaid {
	...
	if (defined $prefix && exists $cdr->{$prefix.'prepaid'} && defined $cdr->{$prefix.'prepaid'}) {
		...
		#   source_prepaid
		#   destination_prepaid <-- mediato should provide this one at least
