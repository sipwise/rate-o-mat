...
sub drop_prepaid_cost {
	my $entry = shift;
	my $result = $sth_delete_prepaid_cost->execute($entry->{call_id})
		or FATAL "Error executing delete prepaid cost statement: ".$sth_delete_prepaid_cost->errstr;
	if (defined $prepaid_costs_cache) {
		delete($prepaid_costs_cache->{$entry->{call_id}});
...
sub get_customer_call_cost
{
	...
	unless($billing_info{profile_id}) {
		...
		DEBUG "no billing profile for ".$dir."customer contract ID $contract_id, skip";
		return -1;
	}
	...
	if ($outgoing_prepaid) { #prepaid out
		...
		if($billing_info{prepaid} == 1 && $direction eq "in") { #prepaid in
...
sub get_provider_call_cost
{
	...
	unless($billing_info{profile_id}) {
		$$r_rating_duration = $cdr->{duration};
		DEBUG "no billing profile for ".$dir."provider contract ID $contract_id, skip";
		return -1;
	}
	...
	while(!$shutdown)
	{
			...
			foreach my $cdr (@cdrs) {
				...
				if ($error) {
					...
					if ($rollback) {
						...
						if (!$terminate_upon_fatal && !defined $DBI::err) {
							WARNING $info_prefix."rating CDR ID $cdr_id aborted - " . $error;
							next; #move on to the next cdr of the batch
						} else {
							die($error); #rethrow
						}
		...
		if ($error)	{
			if (defined $DBI::err) {
				...
				rollback_all();
				if ($terminate_upon_fatal) {
					FATAL $info_prefix."rating CDR ID $cdr_id aborted, please fix it manually - " . $error;
				}
			}
		}
