...
EOS
	...
	$sth_unrated_cdrs = $acctdbh->prepare(
		"SELECT * ".
		#"from accounting.cdr where id in(2604544)" #2604476,2604490,2604544
...
sub get_call_cost {
	...
	while ($duration > 0) {
		...
		if ($split_peak_parts #break the cdr, if
			...
			DEBUG ($$r_onpeak ? 'onpeak' : 'offpeak').' -> '.($onpeak ? 'onpeak' : 'offpeak').' transition, split cdr';
			#$split = 1;
			last;
		}
		$rate *= $interval;
	...
	#if (defined $cdr->{rating_duration}) {
	#	#$interval = $onpeak == 1 ?
	#	#	$r_profile_info->{on_follow_interval} : $r_profile_info->{off_follow_interval};
	#	if ($$r_rating_duration < $cdr->{rating_duration}
	#		and ($cdr->{rating_duration} - $$r_rating_duration) % $interval == 0) {
	#		$$r_rating_duration = $cdr->{rating_duration};
	#	}
	#} else {
	#	$cdr->{rating_duration} = $$r_rating_duration;
	#}
	#if (defined $cdr->{rating_duration}) {
	#	$$r_rating_duration = $cdr->{rating_duration} if $cdr->{rating_duration} >= $cdr->{duration};
	#	#if ($duration > 0)
	#	#if (not $split and $cdr->{rating_duration} < $cdr->{duration}) {
	#	#	$$r_rating_duration = $cdr->{rating_duration};
	#	#}
	#	#if $$r_rating_duration < $cdr->{duration}; #$$r_rating_duration < $cdr->{rating_duration};
	#} elsif ($split_peak_parts) {
	#	$cdr->{rating_duration} = $$r_rating_duration;
	#}
	if (defined $cdr->{rating_duration}) {
		$$r_rating_duration = $cdr->{rating_duration} if $cdr->{rating_duration} > $$r_rating_duration;
	}
...
RATING_DURATION_FOUND:
	...
	$readonly = ($split_peak_parts ? 0 : ($rating_attempts == 1));
	...
	if ($split_peak_parts) {
		...
		if (scalar(keys(%rating_durations)) > 1) {
			if ($rating_attempts > 1) {
				...
				DEBUG 'Inconsistend rating fragment durations '.join(', ',keys(%rating_durations))." for cdr ID $cdr->{id}, trying again";
