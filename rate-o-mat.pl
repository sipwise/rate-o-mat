...
my $sth_get_cdr_cash_balance_cols;
my $sth_get_cdr_time_balance_cols;
my $sth_get_cdr_billing_fk_cols;
my $sth_write_cdr_cash_balance_col_data;
my $sth_write_cdr_time_balance_col_data;
my $sth_write_cdr_time_balance_col_data;
...
EOS
	...
	$cdr_col_models{$cash_balance_col_model_key} = { description => 'cdr cash balance', };
	$cdr_col_models{$time_balance_col_model_key} = { description => 'cdr time balance', };
	$cdr_col_models{$billing_fk_col_model_key} = { description => 'cdr billing foreign key', };

	$cdr_col_models{$cash_balance_col_model_key}->{get_cols_sth} = $acctdbh->prepare(
		"SELECT * FROM accounting.cdr_cash_balance_col"
	) or FATAL "Error preparing get ".$cdr_col_models{$cash_balance_col_model_key}->{description}." cols statement: ".$acctdbh->errstr;
	$cdr_col_models{$time_balance_col_model_key}->{get_cols_sth} = $acctdbh->prepare(
		"SELECT * FROM accounting.cdr_time_balance_col"
	) or FATAL "Error preparing get ".$cdr_col_models{$time_balance_col_model_key}->{description}." cols statement: ".$acctdbh->errstr;
	$cdr_col_models{$billing_fk_col_model_key}->{get_cols_sth} = $acctdbh->prepare(
		"SELECT * FROM accounting.cdr_billing_fk_col"
	) or FATAL "Error preparing get ".$cdr_col_models{$billing_fk_col_model_key}->{description}." cols statement: ".$acctdbh->errstr;

	$cdr_col_models{$cash_balance_col_model_key}->{write_col_data_sth} = $acctdbh->prepare(
		"INSERT INTO accounting.cdr_cash_balance_col_data".
		"  (id,cdr_id,col_id,val_before,val_after) VALUES".
		"  (null,?,?,?,?) ON DUPLICATE KEY UPDATE ".
		"val_before = ?, val_after = ?"
	) or FATAL "Error preparing write ".$cdr_col_models{$cash_balance_col_model_key}->{description}." col data statement: ".$acctdbh->errstr;
	$cdr_col_models{$time_balance_col_model_key}->{write_col_data_sth} = $acctdbh->prepare(
		"INSERT INTO accounting.cdr_time_balance_col_data".
		"  (id,cdr_id,col_id,val_before,val_after) VALUES".
		"  (null,?,?,?,?) ON DUPLICATE KEY UPDATE ".
		"val_before = ?, val_after = ?"
	) or FATAL "Error preparing write ".$cdr_col_models{$time_balance_col_model_key}->{description}." col data statement: ".$acctdbh->errstr;
	$cdr_col_models{$billing_fk_col_model_key}->{write_col_data_sth} = $acctdbh->prepare(
		"INSERT INTO accounting.cdr_billing_fk_col_data".
		"  (id,cdr_id,col_id,fk_id) VALUES".
		"  (null,?,?,?) ON DUPLICATE KEY UPDATE ".
		"fk_id = ?"
	) or FATAL "Error preparing write ".$cdr_col_models{$billing_fk_col_model_key}->{description}." col data statement: ".$acctdbh->errstr;
...
sub update_cdr {
	...
	if ($sth->rows > 0) {
		...
		foreach my $dir (('source_', 'destination_')) {
			foreach my $type (('carrier_','reseller_','customer_')) {
				write_cdr_col_data($cash_balance_col_model_key,$cdr->{id},
					$dir.$type."cash_balance",
					$cdr->{$dir.$type."cash_balance_before"},
					$cdr->{$dir.$type."cash_balance_after"});

				write_cdr_col_data($time_balance_col_model_key,$cdr->{id},
					$dir.$type."free_time_balance",
					$cdr->{$dir.$type."free_time_balance_before"},
					$cdr->{$dir.$type."free_time_balance_after"});

				write_cdr_col_data($billing_fk_col_model_key,$cdr->{id},
					$dir.$type."profile_package_id",
					$cdr->{$dir.$type."profile_package_id"});

				write_cdr_col_data($billing_fk_col_model_key,$cdr->{id},
					$dir.$type."contract_balance_id",
					$cdr->{$dir.$type."contract_balance_id"});
			}
		}
	} else {
		$rollback = 1;
...
sub populate_cdr_cols_map {

	my $cdr_col_model_key = shift;
	FATAL "unknown column model key $cdr_col_model_key" unless exists $cdr_col_models{$cdr_col_model_key};
	my $sth = $cdr_col_models{$cdr_col_model_key}->{get_cols_sth};
	my $description = $cdr_col_models{$cdr_col_model_key}->{description};
	$sth->execute()
		or FATAL "Error executing get $description cols statement: ".$sth->errstr;
	my $cols_map =  $sth->fetchall_hashref('col');
	$sth->finish;
	$cdr_col_models{$cdr_col_model_key}->{cols_map} = $cols_map;
	INFO "$description columns map loaded\n";
	return $cols_map;
...
sub write_cdr_col_data {
	...
	if ((scalar @vals) == 0 || (scalar grep { defined $_ } @vals) == 0) {
        DEBUG "empty '$col_name' $description col data for cdr id $cdr_id, skipping";
		return 0;
    }
	my $sth = $cdr_col_models{$cdr_col_model_key}->{write_col_data_sth};
	my $cols_map = $cdr_col_models{$cdr_col_model_key}->{cols_map};
	my $col_id = $cols_map->{$col_name};
	$sth->execute($cdr_id, $col_id, @vals)
		or FATAL "Error executing write $description col data statement: ".$sth->errstr;
	if ($sth->rows > 0) {
		DEBUG "$description col data written for cdr id $cdr_id, column '$col_name': ".join(', ',@vals);
	} else {
		$rollback = 1;
		FATAL "failed to write $description col data for cdr id $cdr_id, column '$col_name': ".join(', ',@vals);
	}
	return $sth->rows;
...
sub debug_rating_time {
	...
	my $cdr_id = shift;
	my $error = shift;
	DEBUG sub { "rating cdr ID $cdr_id " . ($error ? "aborted after" : "completed sucessfully in") . ' ' . sprintf("%.3f",Time::HiRes::time() - $t) . " secs" };
...
sub main {
	...
	my $cash_balance_col_model_key = 1;
	populate_cdr_cols_map($cash_balance_col_model_key);
	populate_cdr_cols_map($time_balance_col_model_key);
	populate_cdr_cols_map($billing_fk_col_model_key);
	...
	foreach (keys %cdr_col_models) {
		$cdr_col_models{$_}->{get_cols_sth}->finish;
		$cdr_col_models{$_}->{write_col_data_sth}->finish;
	}
