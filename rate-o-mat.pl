...
sub prepare_cdr_col_models {
	...
	$cdr_col_models{$col_model_key} = { description => $model_description, };
...
sub write_cdr_col_data {
	...
	foreach my $dimension (@{$model->{dimensions}}) {
		...
		unless ($dimension_value) {
			$rollback = 1;
			FATAL "missing '$dimension' dimension for writing col data of ".$model->{description};
		}
		...
		unless ($dimension_value_lookup) {
			$rollback = 1;
			FATAL "unknown '$dimension' col name '$dimension_value' for writing col data of ".$model->{description};
		}
	...
	$sth->execute(@bind_parms)
		...
		."statement: ".$sth->errstr;
	if ($sth->rows > 0) {
		DEBUG 'col data written for cdr id '.$cdr->{id}.", column '$virtual_col_name': ".join(', ',@vals);
	} else {
		$rollback = 1;
		FATAL 'no col data written for cdr id '.$cdr->{id}.", column '$virtual_col_name': ".join(', ',@vals);
	}
	return $sth->rows;
